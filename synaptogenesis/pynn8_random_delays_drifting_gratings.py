...
if not args.testing:
    ...
    if args.topology == 2 or args.topology == 3:
        ...
        inh_inh_projection = sim.Projection(
            inh_pop, inh_pop,
            sim.FixedProbabilityConnector(0.),
            synapse_type=inh_structure_model_w_stdp,
...
try:
    ...
    if args.record_source:
        ...
        pre_off_spikes = source_pop_off.spinnaker_get_data('spikes')
        pre_noise_spikes = noise_pop.spinnaker_get_data('spikes')
    else:
        pre_spikes = []
    ...
    if args.topology != 1 and args.record_inh and args.testing:
        inh_post_spikes = inh_pop.spinnaker_get_data('spikes')
    else:
        inh_post_spikes = []
    # End simulation on SpiNNaker
    if args.record_exc_v:
        exc_v_recording = target_pop.get_data('v')
    sim.end()
