...
def weight_shuffle(conn, weights, area):
    ...
    for post_id in range(weights_copy.shape[1]):
        ...
        within_row_filter = np.argwhere(np.logical_and(pre_ids>=0, pre_ids<=255))
        permutation = np.random.permutation(within_row_filter)
        for index in range(within_row_filter.size):
            weights_copy[permutation[index], post_id] = weights[within_row_filter[index], post_id]
...
for file in paths:
    try:
        ...
        if not args.plot:
            ...
            final_mean_number_ff_synapses = number_ff_incoming_connections / float(N_layer)
            ...
            gen_init_conn, gen_init_weight = \
                list_to_post_pre(np.asarray(generated_ff_conn), np.asarray(generated_lat_conn), s_max, N_layer)
            ...
            # d
            final_fan_in_weight = fan_in(last_conn, last_weight, 'weight', 'ff')
